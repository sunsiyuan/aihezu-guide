...
## 算法优化 / 多 Step 场景下的省额度建议

如果你是在做算法优化、性能调优、benchmark 对比、长链路排错，这类任务会比普通问答更容易快速烧额度。
...
这样可以显著减少历史上下文不断膨胀的问题。

#### 3. 日志和 benchmark 先压缩，再喂给模型

不要直接贴原始长日志、长 trace、全量 benchmark 输出。
...
- 不是“做了 10 个 step”
- 而是“做了 10 个越来越贵的 step”

因为上下文会累积，后面的几轮通常比前面的几轮更贵。
