...
## Step 7：当前案例里，400 / 402 先不再作为主排查线

在你们当前这组案例里：

- `402` 已确认是额度问题
- `400` 也按同一类问题先处理，不再优先怀疑网络 / hosts / TUN

所以这两类报错在当前排查里先按下面处理：

1. 先查额度
2. 再看是不是 `opus`
3. 再看是不是长会话 / 大上下文 / 多 step 导致快速耗尽

只有在额度明确正常、而且 terminal 与 IDE 表现仍然明显不一致时，才再单独怀疑 IDE 集成兼容性。
...
那当前就不要继续把 400 / 402 当成网络根因排查，先把主精力放在：
